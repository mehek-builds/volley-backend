...
describe('402 quota payload (R-043)', () => {
  test('resumes, free tier, live link configured: Litos Premium copy plus the Upgrade sentence', () => {
    withUpgradeEnv({ UPGRADE_URL: LIVE_LINK }, () => {
      ...
      assert.equal(
        payload.error,
        `You've used your ${LIMITS.free.monthlyResumes} free resume generations this month. Litos Pro starts at $20/week and includes 1,000 resume generations + autofill. Resets on the 1st. Upgrade: ${LIVE_LINK}`
      );
  ...
  test('resumes, free tier, nothing configured: the Upgrade sentence is omitted and the message still reads cleanly', () => {
    withUpgradeEnv({}, () => {
      ...
      assert.equal(
        payload.error,
        `You've used your ${LIMITS.free.monthlyResumes} free resume generations this month. Litos Pro starts at $20/week and includes 1,000 resume generations + autofill. Resets on the 1st.`
      );
