...
export function quotaExceededPayload(ent: Entitlements, used: number, what: 'contacts' | 'drafts' | 'resumes') {
  ...
  if (what === 'resumes') {
    ...
    const base =
      ent.tier === 'free'
        ? `You've used your ${cap} free resume generations this month. ${PRODUCT_NAME} Pro starts at $20/week and includes ${LIMITS.pro.monthlyResumes.toLocaleString()} resume generations + autofill. Resets on the 1st.`
