 ...
 * dropping it is a compile error - an array where a string belongs - rather than a regression. It
 * is the same lesson `confirmation` below records from the other direction: that one stays optional
 * for the label-only callers, and reviewQuestionRequiresAttention is what makes it undroppable. */
export function sensitiveQuestionRequiresAttention(
  ...
  /* AND THE TWO EXITS DO NOT OVERLAP, WHICH IS WHY THIS ORDER COSTS HER NOTHING.
   *
   * A confirmation is an EARLY EXIT, never a precondition, so a question the resolver can now
   * answer falls straight through the branch above with no confirmation on file and is cleared
   * below. She is not asked to confirm what her own office answers already settled: the two rules
   * cover a sensitive question the resolver CANNOT answer and one it CAN, in that order. */
...
export function reviewQuestionRequiresAttention(
  /* THE WHOLE PACKET, alongside the one record being judged, and both are REQUIRED.
   *
   * The gate now has two ways out and each needs a different input: her confirmation lives on the
   * RECORD, and the country she indicated for this application lives on the FORM. Neither can be an
   * optional trailing argument for the same reason - dropping one is a token, type-checks, and
   * silently reverts half the fix - so both are required and the order between them carries no
   * meaning. The packet leads only because a list reads oddly in the middle of a parameter list. */
  packetQuestions: readonly { question: string; answer?: string }[],
