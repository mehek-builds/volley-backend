...
const PUBLIC_COMPANY_LOGO_KEY = /^company-logos\/rippling\/[a-z0-9](?:[a-z0-9._~-]{0,126}[a-z0-9])?\/[0-9a-f]{64}\.(?:png|jpg|gif|webp)$/;
...
export function isPublicObjectKey(objectKey: string): boolean {
  return PUBLIC_COMPANY_LOGO_KEY.test(objectKey);
}
...
export async function putObject(
  ...
  if (options.publicRead && !isPublicObjectKey(requestedKey)) {
    throw new Error('Refusing public access for an unapproved object key');
  }
  if (provider() === 'vercel') {
  ...
  await client().send(new PutObjectCommand({
    ...
    ContentType: options.contentType,
    ...(options.cacheControlMaxAge === undefined ? {} : {
      CacheControl: `${options.publicRead ? 'public' : 'private'}, max-age=${options.cacheControlMaxAge}${options.publicRead ? ', immutable' : ''}`,
    }),
  }));
  return {
    url: options.publicRead ? publicObjectReadUrl(pathname) : objectReadUrl(pathname),
    pathname,
  };
}
