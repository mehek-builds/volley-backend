...
function pushGreenhouseKnownQuestionAliases(actions: ManagedBrowserAction[], packet: SubmissionPacket) {
  ...
  for (const item of packet.questions) {
    for (const alias of greenhouseKnownQuestionAliases(item.question, item.answer)) {
      ...
      for (const [selectorIndex, selectSelector] of greenhouseQuestionSelectSelectors(alias).entries()) {
        managedSelect(actions, selectSelector, item.answer.trim(), `greenhouse_known_select_${selectorIndex}:${alias.slice(0, 80)}`);
        managedSelect(
          actions,
          selectSelector,
          item.answer.trim().toLowerCase() === 'yes' ? '1' : '0',
          `greenhouse_known_select_value_${selectorIndex}:${alias.slice(0, 80)}`,
        );
      }
    }
  }
