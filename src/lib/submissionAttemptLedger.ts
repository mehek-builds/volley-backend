...
export function confirmedOrphanAttributionForParent(
...

  {
    ...
    const confirmation = exactEvents.find((event) => event.event_kind === 'submission_confirmed');
    if (!opening
      || !confirmation
      || exactEvents.length !== 2
      || exactEvents.filter((event) => event.event_kind === 'attempt_opened').length !== 1
      || exactEvents.filter((event) => event.event_kind === 'submission_confirmed').length !== 1
