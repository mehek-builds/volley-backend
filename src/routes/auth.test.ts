...
// body, and default sender long after the product became RoleQuick - the stale-name class
...
describe('verification email copy (R-044)', () => {
  test('default sender display name is RoleQuick when RESEND_FROM is unset', () => {
    withEnv({ RESEND_FROM: undefined }, () => {
      assert.equal(buildVerificationEmail(EMAIL, CODE).from, 'RoleQuick <onboarding@resend.dev>');
    });
  });

  test('a configured RESEND_FROM is passed through verbatim', () => {
    withEnv({ RESEND_FROM: 'RoleQuick <hi@rolequick.app>' }, () => {
      assert.equal(buildVerificationEmail(EMAIL, CODE).from, 'RoleQuick <hi@rolequick.app>');
    });
  });

  test('subject carries the code and the RoleQuick name', () => {
    assert.equal(buildVerificationEmail(EMAIL, CODE).subject, '123456 is your RoleQuick verification code');
  });

  test('body welcomes the user to RoleQuick, shows the code, and states the 10-minute expiry', () => {
    const { html } = buildVerificationEmail(EMAIL, CODE);
    assert.ok(html.includes('Welcome to RoleQuick.'));
